...
Token 详情现在会从 rollout 只读统计上下文压缩历史，显示累计压缩次数和最近完成时间；没有记录时显示 `0` 和 `-`。当前阶段不会安装或修改 Codex Hook，也不会读取或保存压缩摘要、会话正文、Reasoning、工作目录或 transcript；实时“压缩中”状态将在后续可选 Hook 阶段实现。
