...
发送前必须同时满足：Codex 窗口唯一、通过任务 ID 深链定位、顶部 Rename 标题精确匹配、composer 实例已切换且为空、发送按钮结构唯一。每次只调用一次发送，绝不因超时重试；结果通过目标 rollout 中的精确用户消息与 `task_started` 确认。无人值守恢复结束后，只有当前前台仍是本次 Codex 进程时才尝试回到恢复前的应用；用户主动切换到其他应用时不会抢回焦点。设置与最多 100 条恢复记录只保存在本机，记录不包含提示词、任务标题、Codex 路径、应用身份或原始异常。
