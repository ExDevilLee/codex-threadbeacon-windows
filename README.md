...
发送前必须满足 Codex 窗口唯一。若 Codex 已在前台，只有顶部 Rename 标题唯一精确匹配目标、唯一 composer 可读且为空时才会直接复用；草稿、标题歧义、composer 缺失/重复或内容不可读都会安全停止。其他情况继续通过任务 ID 深链定位，并要求顶部 Rename 标题精确匹配、composer 实例已切换且为空、发送按钮结构唯一。每次只调用一次发送，绝不因超时重试；结果通过目标 rollout 中的精确用户消息与 `task_started` 确认。无人值守恢复结束后，只有当前前台仍是本次 Codex 进程时才尝试回到恢复前的应用；用户主动切换到其他应用时不会抢回焦点。设置与最多 100 条恢复记录只保存在本机；熔断状态只保存任务 ID、异常类型、故障回合 ID、尝试次数和时间；失败记录仅增加稳定诊断码，不包含提示词、任务标题、Codex 路径、composer 内容、应用身份或原始异常。
