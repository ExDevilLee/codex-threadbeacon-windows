...
“通用”页可开启“色盲安全状态标识”。开启后，主任务和 Subagent 会在保留状态颜色与文字的同时，以不同形状区分错误、需要操作、警告、运行中、刚完成、空闲和未知状态。该选项默认关闭、即时生效并在重启后保留；固定大小的状态槽不会因切换而改变列表布局。数据源健康盾牌已经同时使用颜色、形状和文字，因此不受此开关影响。
