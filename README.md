...
右上角扬声器按钮可配置任务完成提示音，并提供与 macOS 版本一致的 Beacon、Chime 和 Pulse 三种内置音色及试听功能。只有自动刷新发现新的可靠 `task_complete` 事件时才播放一次；同一批多个完成事件会合并为一次提示。App 启动、手动刷新和恢复监听时只建立完成事件基线，不会补播历史任务。声音开关、所选音色和最多 256 个派生事件 ID 保存在本机，不保存任务标题、正文、Token 详情或 Codex 路径。

创建过 Subagent 的主任务会在标题右侧显示中性的分支图标和直接 Subagent 总数。该数字来自父子关系表，是历史关系数量，不代表当前正在运行的数量；数量为 0 时不显示且不保留空白。此功能不读取或显示子任务标题、状态、Token、路径或正文。
...
- 在主任务标题右侧显示非零的直接 Subagent 历史总数。
- 在窗口副标题显示运行中主任务数与当前显示总数。
- 每 2 秒自动刷新并支持手动刷新。
- SQLite 全程只读。
- 不读取正文、不访问网络、不修改 Codex 数据。

失败/警告等事件提示音、任务置顶/忽略、Subagent 展开、429/503 和系统托盘仍未实现。
