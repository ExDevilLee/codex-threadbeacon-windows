...
ThreadBeacon 的紧凑列表很适合单独放在 7 英寸等竖向扩展屏上：电脑屏幕保留 Codex 交互，主显示器查看代码和 Diff，小屏则持续显示各主任务的运行、完成与异常状态。
...
累计 Token 后的信息按钮现为“任务详情”，优先显示主任务使用的模型与推理强度，并继续显示会话总量、输入、缓存输入、非缓存输入、输出、Reasoning、当前 turn、缓存率和更新时间。模型与推理强度优先取自只读任务数据库，缺失字段才由 rollout 中最近有效的 `turn_context` 补充；悬停会短暂显示详情，点击可固定弹窗，任务列表自动刷新时已打开的固定弹窗保持稳定。

右上角齿轮按钮打开独立设置窗口。“通用”页可将刷新间隔设为 1、2、5 或 10 秒，将“刚完成”状态保留时间设为 1 至 5 分钟，并将主窗口最大任务数设为 4、8、12 或 20；修改会立即保存并生效，且不会改变暂停状态。保留时间同时用于主任务和展开的 Subagent；调整时只执行一次基线刷新，不会重放完成提示音或历史自动恢复事件。“提示音”页提供与 macOS 版本一致的八种内置音色及试听功能。新安装默认使用 Chime 作为完成音、Alert 作为 429/503 异常音，两类通知都可独立选择八种音色中的任一种。只有自动刷新发现新的可靠 `task_complete` 事件时才播放一次；同一批多个完成事件会合并为一次提示。App 启动、手动刷新、恢复监听和任务数量调整时只建立完成事件基线，不会补播历史任务。显示偏好保存在 `%LOCALAPPDATA%\ThreadBeacon\display-settings.json`，声音开关、所选音色和最多 256 个派生事件 ID 也只保存在本机；这些文件不保存任务标题、正文、Token 详情或 Codex 路径。
...
标题栏的信息按钮打开单实例“关于”窗口，显示 App 图标、当前运行版本、项目定位和非官方说明，并提供 GitHub、Releases、隐私说明、MIT License 与“支持项目”入口。支持页提供 Star、分享、Issue、贡献代码以及完全自愿的微信支付和支付宝赞助方式；赞助不解锁功能、不影响免费使用，付款二维码不会出现在 App 主窗口。只有用户主动点击这些链接时才会交给默认浏览器打开。
...
App 现在也会监控当前可见主任务的 HTTP 400/429/503 服务异常、模型容量不足错误和重连耗尽后的最终连接中断。HTTP 429/503 重试阶段以黄色“服务异常”显示状态码与重试进度；HTTP 400、重试耗尽、模型容量不足，或同一 turn 的 `5/5` 重连后出现最终断流时变为红色“服务失败”。最终连接中断会显示“连接中断 · 重试 5/5”；单独的 `5/5` 仍保持黄色警告，缺少耗尽证据的断流文本不会被推测为终止失败。同一 turn 后续 HTTP 200 或较新的 rollout 生命周期事件会清除旧异常。每个异常 episode 最多播放一次独立可配置的提示音；异常与完成事件共享基线和 256 条本地派生 ID 历史。

“提示音”页支持为完成和服务异常分别选择本地 WAV 文件、试听和清除。自定义文件不可访问或格式无效时，会自动回退到各自选择的内置音色。

创建过 Subagent 的主任务会在标题右侧显示中性的分支图标和 `运行中数/历史总数`，例如 `2/27`；没有直接 Subagent 时不显示且不保留空白。分子只统计 rollout 经现有 120 秒新鲜度规则确认仍为 `Running` 的直接子任务，不使用关系表的 `open` 字段，也不递归。为控制每 2 秒刷新成本，折叠状态只查询最近 120 秒更新的候选并读取其 rollout 尾部；点击徽标后按需读取完整直接子任务详情，且同轮刷新复用已经解析的候选。展开行显示 `表意任务名 | 标题`、推导状态、最近活动和累计 Token：表意任务名优先从 `agent_path` 最后一段生成，例如 `/root/fix_external_sync` 显示为 `Fix external sync`，旧记录缺少该字段时回退到 Agent nickname。数据库缺列时安全降级；详情中显示角色、模型、Reasoning 与 Token 数字明细，不读取正文。
...
- 显示主任务模型、推理强度和累计 Token，并提供不含正文的任务详情弹窗。
- 对自动刷新发现的新任务完成事件播放可配置的内置提示音。
- 从只读本地日志中识别当前可见主任务的 HTTP 400/429/503 与模型容量异常。
- 在主任务标题右侧显示直接 Subagent 的运行中数/历史总数，并支持按需行内展开。
...
设置窗口的“自动恢复”页提供默认关闭的续接能力，覆盖终止型 HTTP 400、HTTP 429、其他 HTTP 4xx/5xx、模型容量异常和重连耗尽后的最终连接中断，HTTP 503 默认关闭。每类异常可独立启用并配置不超过 500 字的提示词；连接中断规则默认启用，但只有总开关由用户明确开启后才会执行。每条规则还可启用连续恢复熔断器，默认在同一任务、同类异常连续尝试 3 次后停止，可配置为 1 至 20 次。当前熔断可在设置页手动解除；目标任务出现更新的确认完成事件后也会自动恢复。App 启动时只建立基线，不补发历史故障。

发送前必须满足 Codex 窗口唯一。若 Codex 已在前台且顶部 Rename 标题唯一精确匹配目标，只有唯一 composer 可读且为空时才会直接复用。若前台当前不是目标任务，也只有来源 composer 唯一、可读且为空时才允许通过任务 ID 深链切换；来源存在草稿、缺失、重复或内容不可读都会安全停止。导航后仍要求顶部 Rename 标题精确匹配、composer 实例已切换且为空、发送按钮结构唯一。每次只调用一次发送，绝不因超时重试；结果通过目标 rollout 中的精确用户消息与 `task_started` 确认。无人值守恢复结束后，只有当前前台仍是本次 Codex 进程时才尝试回到恢复前的应用；用户主动切换到其他应用时不会抢回焦点。设置与最多 100 条恢复记录只保存在本机；熔断状态只保存任务 ID、异常类型、故障回合 ID、尝试次数和时间；失败记录仅增加稳定诊断码，不包含提示词、任务标题、Codex 路径、composer 内容、应用身份或原始异常。

双击未归档的主任务行可在已安装的 Codex App 中打开对应任务。导航复用任务 ID 深链、精确 Rename 标题、composer 实例切换和空 composer 预检，但不会输入或发送文字；已归档的收藏观察任务不会触发导航，双击 Subagent 展开或 Token 详情按钮也只执行按钮原有操作。
...
- `docs`：Windows 数据探测、设计记录和 [macOS 对齐记录](docs/macos-parity.md)。
...
脚本会在 `artifacts/release/<tag>` 下生成便携 ZIP 和发布 EXE。推荐分发 ZIP，因为其中包含 EXE、配套的内置提示音资源和可选实时压缩状态使用的 Hook Bridge；单文件 EXE 也会在运行时释放同一 Bridge。
...
- 任务详情仅保留模型名称、推理强度和 Token 数字字段；rollout 中后续空字段不会覆盖最近有效元数据。
- 不启动网络服务，不上传任务数据。
...
“通用”页可调整“色盲安全状态标识”。开启后，主任务和 Subagent 会在保留状态颜色与文字的同时，以不同形状区分错误、需要操作、警告、运行中、已中断、刚完成、空闲和未知状态。新安装以及缺少该字段的旧设置默认开启；用户显式关闭后会即时生效并在重启后保持关闭。固定大小的状态槽不会因切换而改变列表布局。数据源健康盾牌已经同时使用颜色、形状和文字，因此不受此开关影响。
...
Token 详情会从 rollout 只读统计上下文压缩历史，显示累计压缩次数和最近完成时间；没有记录时显示 `0` 和 `-`。

“通用”设置页还提供默认关闭的实时压缩状态。用户主动启用后，ThreadBeacon 会结构化合并 `%CODEX_HOME%\hooks.json` 中的 `PreCompact` 与 `PostCompact`，并把独立 Bridge 安装到 `%LOCALAPPDATA%\ThreadBeacon\hooks\v1`。启用前会备份现有配置；检查按钮可重新验证状态；停用时只删除 ThreadBeacon 自己的两个处理器并保留其他 Hook。若 `hooks.json` 无效、是重解析点、配置在操作期间被修改，或 `config.toml` 已内联定义 Hooks，操作会安全停止而不覆盖原配置。

活动标记只包含会话 ID、turn ID、`manual`/`auto` 触发方式和本机开始时间，并有 15 分钟失效兜底。Bridge 不保存 Hook 载荷中的 transcript 路径、工作目录、模型，也不读取或保存压缩摘要、会话正文或 Reasoning。Codex 可能仍会要求用户审核并信任 Hook 命令。
