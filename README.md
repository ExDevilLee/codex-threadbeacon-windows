...
创建过 Subagent 的主任务会在标题右侧显示中性的分支图标和 `运行中数/历史总数`，例如 `2/27`；没有直接 Subagent 时不显示且不保留空白。分子只统计 rollout 经现有 120 秒新鲜度规则确认仍为 `Running` 的直接子任务，不使用关系表的 `open` 字段，也不递归。为控制每 2 秒刷新成本，折叠状态只查询最近 120 秒更新的候选并读取其 rollout 尾部；点击徽标后按需读取完整直接子任务详情，且同轮刷新复用已经解析的候选。展开行显示 `Agent 别名 | 标题`、推导状态、最近活动和累计 Token，详情中显示角色、模型、Reasoning 与 Token 数字明细；不读取正文。
