...
The Auto recovery tab provides an opt-in continuation workflow for terminal HTTP 400, HTTP 429, other HTTP 4xx/5xx, model-capacity failures, and final stream disconnects after reconnect attempts are exhausted; HTTP 503 remains disabled by default. Every rule has an independent switch and a prompt of at most 500 characters. The connection-interrupted rule defaults on, but it cannot run unless the master switch is explicitly enabled. Each rule also has an optional consecutive-recovery circuit breaker. It defaults to stopping after three distinct episodes for the same task and incident type, supports limits from 1 to 20, can be reset from the settings page, and resets automatically after a newer confirmed completion. Startup establishes a baseline and never replays historical failures. ThreadBeacon always requires exactly one Codex window. If Codex is already foreground and exactly one app-header title matches the expected Rename title, recovery may reuse the current task only when its sole composer is readable and empty. If a different task is foreground, ID deep-link navigation is allowed only when the source composer is also unique, readable, and empty; a source draft, missing or multiple composers, or unreadable content fails closed. Navigation still requires exact target-title confirmation, a changed empty composer, and one structurally verified send button. ThreadBeacon invokes Send once and never retries; the result is confirmed from the target rollout. After unattended recovery, it restores the previously foreground application only if the same Codex process is still foreground; a user focus change is never overridden. Settings and at most 100 history entries stay local. Circuit state stores only the task ID, incident type, episode ID, attempt count, and timestamp. Failed entries add only stable diagnostic codes and exclude prompts, titles, paths, composer text, application identities, UI trees, and raw errors.
...
The General tab offers color-blind-safe status indicators. When enabled, main tasks and Subagents retain their status color and text while using distinct shapes for Error, Action, Warning, Running, Interrupted, Done, Idle, and Unknown. New installations and older settings files without this field default to enabled; an explicitly disabled choice remains disabled across restarts. Changes apply immediately, and the fixed-size status slot prevents layout shifts. The data-source health shield already combines color, shape, and text, so this setting does not change it.
